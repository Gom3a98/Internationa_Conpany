...
<tr>
    ...
    <td>
        <a href="#editCategory{{$category->id}}" id="{{$category->id}}" class="edit" data-toggle="modal"><i
                class="material-icons" data-toggle="tooltip" title="Edit">&#xE254;</i></a>
        <a href="#deleteModal" id="{{$category->id}}" class="delete" data-toggle="modal"><i class="material-icons"
                    ...
                    <form action="/admin/category/{{$category->id}}" enctype="multipart/form-data" method="post">
                        ...
                        <div class="modal-body">
                            <div class="form-group">
                                <label>Name</label>
                                <input type="text" class="form-control updatedName" name="product_name"
                                    value="{{$category->name}}" required="">
                                <input type="hidden" name="category_name" value="categories">
                            </div>
                            <select class="form-control custom-select mr-sm-2" name="type" id="inlineFormCustomSelect">
                                <option value="{{$category->type}}">{{$category->type}}</option>
                            </select>
                            <div class="form-group">
                                <label>img URL</label>
                                <input type="text" class="form-control updatedName" name="imgURL"
                                    value="{{$category->imgURL}}" required="">
                            </div>
                            <div class="form-group">
                                <span class="control-fileupload">
                                    <label for="file1" class="text-left">Please choose a files on your computer.</label>
                                    <input type="file" class="form-control custom-select mr-sm-2"
                                        name="product_images[]" id="file1" multiple>
                                </span>
                            </div>
                        </div>
            ...
            <form action="/admin/category" enctype="multipart/form-data" method="post">
                ...
                <div class="modal-body">
                    ...
                    <div class="col-sm-10">
                        <span class="control-fileupload">
                            <label for="file1" class="text-left">Please choose a files on your computer.</label>
                            <input type="file" required class="form-control custom-select mr-sm-2"
                                name="product_images[]" id="file1" multiple>
                        </span>
                    </div>
                </div>
